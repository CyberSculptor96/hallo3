...
def collect_video_folder_paths(root_path: Path) -> list:
    """收集所有视频文件夹路径"""
    return [frames_dir.resolve() for frames_dir in root_path.iterdir()]


def construct_meta_info(frames_dir_path: Path, json_file: str) -> dict:
    """构建视频帧的元信息"""
    mask_path = str(frames_dir_path).replace("audio_emb", "face_mask") + ".png"
    face_emb_path = str(frames_dir_path).replace("audio_emb", "face_emb") + ".pt"
    audio_emb_path = str(frames_dir_path)
    video_path = str(frames_dir_path).replace("audio_emb", "videos") + ".mp4"
    caption_path = str(frames_dir_path).replace("audio_emb", "captions") + ".txt"
...
def main():
    ...
    parser.add_argument("-i", "--input", type=str, required=True, help="存储视频和caption路径的JSON文件")
    ...
    image_dir = Path(args.root_path) / "audio_emb"
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        futures = {executor.submit(construct_meta_info, path, args.input): path for path in frames_dir_paths}
